...
// SwiftUI representable (VideoScannerView) lives in VideoScanner.swift
...
class VideoCameraViewController: UIViewController {
    ...
    func setActive(_ active: Bool) {
        ...
        if active { startCameraSession() } else { stopCameraSession() }
    }

    private func startCameraSession() {
        guard isActive else { return }
        if previewLayer?.session == nil { previewLayer?.session = captureSession }
        if previewLayer?.superlayer == nil, let pl = previewLayer { view.layer.insertSublayer(pl, at: 0) }
        let session = captureSession
        sessionQueue.async { if session?.isRunning == false { session?.startRunning() } }
    }
    ...
    private func stopCameraSession() {
        ...
        let session = captureSession
        sessionQueue.async { if session?.isRunning == true { session?.stopRunning() } }
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let conn = previewLayer?.connection { conn.isEnabled = true; setPortraitOrientation(on: conn) }
        startCameraSession()
    }

    override func viewDidAppear(_ animated: Bool) { super.viewDidAppear(animated); startCameraSession() }
    override func viewWillDisappear(_ animated: Bool) { super.viewWillDisappear(animated); stopCameraSession() }
    ...
    func setupCamera() {
        ...
        if let aDevice = AVCaptureDevice.default(for: .audio),
           ...
           captureSession.canAddInput(aInput) { captureSession.addInput(aInput) }
    ...
    func setupGestures() {
        ...
        view.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(handleTapFocus(_:))))
    }

    // MARK: - Actions

    @objc func closeTapped() { stopCameraSession(); onClose?() }
    @objc func recordTapped() { isRecording ? stopRecording() : startRecording() }
    ...
    private func startRecording() {
        ...
        if let conn = movieOutput.connection(with: .video), conn.isActive { setPortraitOrientation(on: conn) }
    ...
    private func updateTimerLabel() {
        let m = recordingSeconds / 60; let s = recordingSeconds % 60
    ...
    private func animateInnerCircle(toStop: Bool) {
        ...
        UIView.animate(withDuration: 0.35, delay: 0, usingSpringWithDamping: 0.65, initialSpringVelocity: 0.5, options: []) {
    ...
    @objc func handleTapFocus(_ gesture: UITapGestureRecognizer) {
        ...
        if device.isFocusPointOfInterestSupported && device.isFocusModeSupported(.autoFocus) {
            device.focusPointOfInterest = devicePoint; device.focusMode = .autoFocus
        }
        if device.isExposurePointOfInterestSupported && device.isExposureModeSupported(.autoExpose) {
            device.exposurePointOfInterest = devicePoint; device.exposureMode = .autoExpose
        }
        device.unlockForConfiguration()
    ...
    private func showFocusRing(at point: CGPoint) {
        ...
        focusRingView = ring
        UIView.animate(withDuration: 0.25, animations: { ring.transform = .identity }) { _ in
            UIView.animate(withDuration: 0.4, delay: 0.3) { ring.alpha = 0 } completion: { _ in ring.removeFromSuperview() }
        }
    }
}
...
extension VideoCameraViewController: AVCaptureFileOutputRecordingDelegate {
    func fileOutput(_ output: AVCaptureFileOutput,
        ...
        DispatchQueue.main.async { [weak self] in
            guard let self, let callback = self.onRecordingFinished else { return }
            if let error = error {
                let nsErr = error as NSError
                let ok = (nsErr.userInfo[AVErrorRecordingSuccessfullyFinishedKey] as? Bool) ?? false
                if !ok { print("Video recording ended with error: \(error.localizedDescription)"); return }
            }
